...
class SonicDBConfig(object):
    ...
    def get_sonic_db_socket(db_name):
        ...
        return SonicDBConfig._sonic_db_config["INSTANCES"][inst_name]["unix_socket_path"]

    @staticmethod
    def get_sonic_db_hostname(db_name):
        if SonicDBConfig._sonic_db_config_init == False:
            SonicDBConfig.load_sonic_db_config()
        inst_name = SonicDBConfig._sonic_db_config["DATABASES"][db_name]["instance"]
        return SonicDBConfig._sonic_db_config["INSTANCES"][inst_name]["hostname"]

    @staticmethod
    def get_sonic_db_port(db_name):
        if SonicDBConfig._sonic_db_config_init == False:
            SonicDBConfig.load_sonic_db_config()
        inst_name = SonicDBConfig._sonic_db_config["DATABASES"][db_name]["instance"]
...
class SonicV2Connector(DBInterface):
    ...
    def get_db_list(self):
        return SonicDBConfig.get_sonic_db_list()

    def get_db_socket(self, db_name):
        return SonicDBConfig.get_sonic_db_socket(db_name)

    def get_db_hostname(self, db_name):
        return SonicDBConfig.get_sonic_db_hostname(db_name)

    def get_db_port(self, db_name):
        return SonicDBConfig.get_sonic_db_port(db_name)

    def get_dbid(self, db_name):
        return SonicDBConfig.get_sonic_db_id(db_name)

    def get_db_separator(self, db_name):
        return SonicDBConfig.get_sonic_db_separator(db_name)
